...
async function initDefaults() {
  if (!('ai' in self)) {
    showResponse('Error: self.ai not supported in this browser');
    return;
  }
  const defaults = await self.ai.languageModel.params();
...
function showResponse(response) {
  ...
  const markdownHtml = parseMarkdown(response);
  const sanitizedHtml = sanitizeHtml(markdownHtml);
  elementResponse.innerHTML = sanitizedHtml;
}
...
function sanitizeHtml(html) {
  const allowedTags = ['p', 'ul', 'ol', 'li', 'b', 'i', 'em', 'strong', 'a'];
  const allowedAttributes = {
    a: ['href', 'title'],
  };
  const sanitizedHtml = html.replace(/<\/?[^>]+(>|$)/g, (tag) => {
    if (allowedTags.includes(tag.slice(1, -1))) {
      return tag;
    }
    return '';
  });
...
function parseMarkdown(markdown) {
  const markdownHtml = markdown.replace(/^(#+) (.*)$/gm, (match, p1, p2) => {
